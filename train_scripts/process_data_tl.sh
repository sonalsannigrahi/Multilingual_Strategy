...
BPESIZES="4000 8000 16000 32000" # possibility to add more
TEMPS="1.5"
...
for pair in "${PAIRS[@]}"; do
	...
	for lang in $src $trg; do
	    ...
	    if [ ! -f $datadir/$pair/$dset.$pair.norm.$lang.transliterated.hi ]; then
		cat $datadir/$pair/$dset.$pair.$lang.transliterated.hi | perl $MOSESSCRIPTS/tokenizer/normalize-punctuation.perl $lang \
		    > $datadir/$pair/$dset.$pair.norm.$lang.transliterated.hi
	    fi
	done
	if [ $dset == "train" ]; then
	    # filter out unwanted sentences and deduplicate in train only
	    if [ ! -f $datadir/$pair/$dset.$pair.norm.clean.dedup.$src-$trg.transliterated.hi ]; then
		paste $datadir/$pair/train.$pair.norm.$src.transliterated.hi $datadir/$pair/train.$pair.norm.$trg.transliterated.hi | \
		    python $thisdir/clean_parallel.py -l1 $src -l2 $trg | sort -u \
		    > $datadir/$pair/$dset.$pair.norm.clean.dedup.$src-$trg.transliterated.hi
	    fi
	    if [ ! -f $datadir/$pair/$dset.$pair.norm.clean.dedup.$src.transliterated.hi ]; then
		cat $datadir/$pair/$dset.$pair.norm.clean.dedup.$src-$trg.transliterated.hi | cut -f 1 > $datadir/$pair/$dset.$pair.norm.clean.dedup.$src.transliterated.hi
	    fi
	    if [ ! -f $datadir/$pair/$dset.$pair.norm.clean.dedup.$trg.transliterated.hi ]; then
		cat $datadir/$pair/$dset.$pair.norm.clean.dedup.$src-$trg.transliterated.hi | cut -f 2 > $datadir/$pair/$dset.$pair.norm.clean.dedup.$trg.transliterated.hi
	    fi
	    echo ">> Linking final raw datasets"
	    ln -sf $datadir/$pair/$dset.$pair.norm.clean.dedup.$src-$trg.transliterated.hi $datadir/$pair/$dset.$pair.final.$src-$trg.transliterated.hi
	    ln -sf $datadir/$pair/$dset.$pair.norm.clean.dedup.$src.transliterated.hi $datadir/$pair/$dset.$pair.final.$src.transliterated.hi
	    ln -sf $datadir/$pair/$dset.$pair.norm.clean.dedup.$trg.transliterated.hi $datadir/$pair/$dset.$pair.final.$trg.transliterated.hi
	else
	    ln -sf $datadir/$pair/$dset.$pair.norm.$src.transliterated.hi $datadir/$pair/$dset.$pair.final.$src.transliterated.hi
            ln -sf $datadir/$pair/$dset.$pair.norm.$trg.transliterated.hi $datadir/$pair/$dset.$pair.final.$trg.transliterated.hi
	fi
    done
done
