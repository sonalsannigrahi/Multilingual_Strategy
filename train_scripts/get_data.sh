...
PAIRS=('hi-en' 'ne-en' 'gu-en' 'mr-en' 'be-en')
...
if [ ! -f $datadir/gu-en/train.gu-en.en ]; then
    ...
    [ -d $datadir/raw_corpora/flores.tar.gz ] || wget -O $datadir/raw_corpora/flores.tar.gz https://dl.fbaipublicfiles.com/flores101/dataset/flores101_dataset.tar.gz --no-check-certificate
    [ -f $datadir/raw_corpora/flores101_dataset ] || tar -xvzf $datadir/raw_corpora/flores.tar.gz -C $datadir/raw_corpora/
    
    mv $datadir/raw_corpora/flores101_dataset/dev/guj.dev $datadir/gu-en/dev.gu-en.gu
    mv $datadir/raw_corpora/flores101_dataset/dev/eng.dev $datadir/gu-en/dev.gu-en.en
    mv $datadir/raw_corpora/flores101_dataset/devtest/guj.devtest $datadir/gu-en/test.gu-en.gu
    mv $datadir/raw_corpora/flores101_dataset/devtest/eng.devtest $datadir/gu-en/test.gu-en.en


### Nepali training data
# TODO: add Nepali PTB data?
if [ ! -f $datadir/ne-en/train.ne-en.en ]; then
    wget -O $datadir/ne-en/raw_corpora/bible-uedin.zip https://object.pouta.csc.fi/OPUS-bible-uedin/v1/moses/en-ne.txt.zip --no-check-certificate
    wget -O $datadir/ne-en/raw_corpora/TED2020.zip https://object.pouta.csc.fi/OPUS-TED2020/v1/moses/en-ne.txt.zip --no-check-certificate
    wget -O $datadir/ne-en/raw_corpora/QED.zip https://object.pouta.csc.fi/OPUS-QED/v2.0a/moses/en-ne.txt.zip --no-check-certificate
    wget -O $datadir/ne-en/raw_corpora/GlobalVoices.zip https://opus.nlpl.eu/download.php?f=GlobalVoices/v2018q4/moses/en-ne.txt.zip --no-check-certificate
    wget -O $datadir/ne-en/raw_corpora/GNOME.zip https://opus.nlpl.eu/download.php?f=GNOME/v1/moses/en_GB-ne.txt.zip --no-check-certificate
    wget -O $datadir/ne-en/raw_corpora/KDE4.zip https://opus.nlpl.eu/download.php?f=KDE4/v2/moses/en-ne.txt.zip --no-check-certificate
...
if [ ! -f $datadir/hi-en/train.hi-en.en ]; then
    [ -f $datadir/hi-en/raw_corpora/iitb.zip ] || wget -O $datadir/hi-en/raw_corpora/iitb.zip https://www.cfilt.iitb.ac.in/~parallelcorp/iitb_en_hi_parallel/iitb_corpus_download/parallel.zip --no-check-certificate
    ...
    [ -f $datadir/hi-en/raw_corpora/hindencorp.tsv.gz ] || wget -O $datadir/hi-en/raw_corpora/hindencorp.tsv.gz https://lindat.mff.cuni.cz/repository/xmlui/bitstream/handle/11858/00-097C-0000-0023-625F-0/hindencorp05.plaintext.gz --no-check-certificate
    
    # for hindi-specific corpora
    for corpus in raw_corpora/wikititles-wmt14; do
	[ -d $datadir/$corpus ] || mkdir $datadir/$corpus
	tar -zxvf $datadir/$corpus.tgz -C $datadir/$corpus
    done

    for corpus in hi-en/raw_corpora/iitb; do
    [ -d $datadir/$corpus ] || mkdir $datadir/$corpus
    unzip $datadir/$corpus.zip -d $datadir/$corpus
    done
    
    for lang in en hi; do
	cat $datadir/hi-en/raw_corpora/iitb/parallel-n/IITB.en-hi.$lang >> $datadir/hi-en/train.hi-en.$lang
    done
...
if [ ! -f $datadir/hi-en/raw_corpora/iitb-dev.hi-en.en ]; then
    [ -f $datadir/hi-en/raw_corpora/iitb-devtest.zip ] || wget -O $datadir/hi-en/raw_corpora/iitb-devtest.zip https://www.cfilt.iitb.ac.in/~parallelcorp/iitb_en_hi_parallel/iitb_corpus_download/dev_test.zip --no-check-certificate
        unzip $datadir/hi-en/raw_corpora/iitb-devtest.zip -d $datadir/hi-en
    for dset in dev test; do
	for lang in en hi; do
...
### Marathi dev/test data
if [ ! -f $datadir/mr-en/dev.mr-en.en ]; then
    [ -d $datadir/raw_corpora/flores.tar.gz ] || wget -O $datadir/raw_corpora/flores.tar.gz https://dl.fbaipublicfiles.com/flores101/dataset/flores101_dataset.tar.gz --no-check-certificate
    [ -f $datadir/raw_corpora/flores101_dataset ] || tar -xvzf $datadir/raw_corpora/flores.tar.gz -C $datadir/raw_corpora/
    
    mv $datadir/raw_corpora/flores101_dataset/dev/mar.dev $datadir/mr-en/dev.mr-en.mr
    mv $datadir/raw_corpora/flores101_dataset/dev/eng.dev $datadir/mr-en/dev.mr-en.en
    mv $datadir/raw_corpora/flores101_dataset/devtest/mar.devtest $datadir/mr-en/test.mr-en.mr
    mv $datadir/raw_corpora/flores101_dataset/devtest/eng.devtest $datadir/mr-en/test.mr-en.en
fi

### Bengali dev/test data
#if [ ! -f $datadir/mr-en/dev.be-en.en ]; then
    [ -d $datadir/raw_corpora/flores.tar.gz ] || wget -O $datadir/raw_corpora/flores.tar.gz https://dl.fbaipublicfiles.com/flores101/dataset/flores101_dataset.tar.gz --no-check-certificate
    [ -f $datadir/raw_corpora/flores101_dataset ] || tar -xvzf $datadir/raw_corpora/flores.tar.gz -C $datadir/raw_corpora/
    
    mv $datadir/raw_corpora/flores101_dataset/dev/ben.dev $datadir/be-en/dev.be-en.be
    mv $datadir/raw_corpora/flores101_dataset/dev/eng.dev $datadir/be-en/dev.be-en.en
    mv $datadir/raw_corpora/flores101_dataset/devtest/ben.devtest $datadir/be-en/test.be-en.be
    mv $datadir/raw_corpora/flores101_dataset/devtest/eng.devtest $datadir/be-en/test.be-en.en
#fi
